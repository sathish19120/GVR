...
function EditUserModal({ user: u, onClose, onSaved }) {
  ...
  // ✅ FIX: this state was completely missing. The branch picker below
  // called setBranch(b) on every click, but with no useState for it,
  // that threw "ReferenceError: setBranch is not defined" the instant
  // an admin edited any branch_executive user (or switched a user's
  // role to branch_executive). Initialized from u.branch so the current
  // assignment shows correctly when the modal opens.
  const [branch, setBranch]     = useState(u.branch || '')
  const [saving, setSaving]     = useState(false)
  const [error, setError]       = useState('')

  async function save() {
    // ✅ FIX: require a branch when role is branch_executive, matching
    // the same validation CreateUserModal already has — previously this
    // modal had no such check, so an executive could be saved with role
    // set but no branch attached at all.
    if (role === 'branch_executive' && !branch) {
      setError('Please select a branch for branch executive')
      return
    }
    setSaving(true); setError('')
    try {
      // ✅ FIX: `branch` was never included in this update. Even after
      // fixing the setBranch crash, the picked branch would have been
      // silently discarded — the user's branch column stayed whatever
      // it was before (or null), so BranchDashboard's `user?.branch`
      // lookup for this executive would remain wrong or empty.
      // When switching a user AWAY FROM branch_executive, branch is
      // cleared to null so stale branch data doesn't linger on a role
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:100, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div style={{ background:G.white, borderRadius:20, width:'100%', maxWidth:460, padding:28, maxHeight:'90vh', overflowY:'auto' }}>
        ...
        <div style={{ display:'grid', gap:16 }}>
          ...
          {/* ✅ FIX: moved below Role so it correctly reflects the CURRENT
              value of `role` state (was previously rendered above the
              Role picker, checking a `role` value that hadn't been
              declared yet in the component's read order — harmless by
              itself, but confusing given the crash it was tangled up
              with; now ordered to match CreateUserModal's layout). */}
          {role === 'branch_executive' && (
            <Field label="Assign Branch *">
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                {BRANCHES.map(b => (
