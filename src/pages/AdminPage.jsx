...
// FIX #2: safe fallback so unknown roles never crash
const ROLE_COLORS = {
...
const getRoleColor = (role) => ROLE_COLORS[role] || [G.muted, '#F3F4F6']
...
function CreateUserModal({ onClose, onSaved }) {
  ...
  async function save() {
    ...
    try {
      ...
      // FIX #4: auto-generate referral_code on user creation
      ...
      const { error: err } = await supabase.from('profiles').insert({
        ...
        role, phone: phone || null, branch: branch || null,
      ...
      <div style={{ background:G.white,borderRadius:20,width:'100%',maxWidth:460,padding:28,maxHeight:'90vh',overflowY:'auto' }}>
        ...
        <div style={{ display:'grid',gap:16 }}>
          <Field label="Full Name">
            <input type="text" value={fullName} onChange={e=>setFullName(e.target.value)} placeholder="Enter full name" style={inp} onFocus={e=>e.target.style.borderColor=G.green} onBlur={e=>e.target.style.borderColor=G.border} />
          </Field>
          <Field label="Username *">
            <input type="text" value={username} onChange={e=>setUsername(e.target.value.trim().toLowerCase())} placeholder="Enter username (no spaces)" style={inp} onFocus={e=>e.target.style.borderColor=G.green} onBlur={e=>e.target.style.borderColor=G.border} />
          </Field>
          <Field label="Password *">
            <div style={{ position:'relative' }}>
              <input type={showPass?'text':'password'} value={password} onChange={e=>setPassword(e.target.value)} placeholder="Min 6 characters" style={{ ...inp,paddingRight:50 }} onFocus={e=>e.target.style.borderColor=G.green} onBlur={e=>e.target.style.borderColor=G.border} />
          ...
          <Field label="Role">
            <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:8 }}>
              {ROLES.map(r => {
                const [color, bg] = getRoleColor(r)
                return (
                  <button key={r} type="button" onClick={()=>{ setRole(r); if(r!=='branch_executive') setBranch('') }} style={{ padding:'10px',borderRadius:10,border:`2px solid ${role===r?color:G.border}`,background:role===r?bg:G.white,cursor:'pointer',fontWeight:600,fontSize:12,color:role===r?color:G.muted,textTransform:'capitalize' }}>
                    {r.replace('_',' ')}
                  </button>
                )
              })}
          ...
          {role === 'branch_executive' && (
            <Field label="Assign Branch *">
              <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:8 }}>
                {BRANCHES.map(b => (
                  <button key={b} type="button" onClick={()=>setBranch(b)} style={{ padding:'9px 8px',borderRadius:9,border:`2px solid ${branch===b?'#0891B2':G.border}`,background:branch===b?'#ECFEFF':G.white,cursor:'pointer',fontSize:12,fontWeight:600,color:branch===b?'#0891B2':G.muted }}>{b}</button>
                ))}
              </div>
        ...
        <button type="button" onClick={save} disabled={saving} style={{ width:'100%',marginTop:24,padding:13,background:saving?'#9CA3AF':G.green,color:G.white,border:'none',borderRadius:12,fontSize:15,fontWeight:700,cursor:saving?'not-allowed':'pointer' }}>
...
// ── Edit User Modal ───────────────────────────────────────
// FIX #1: branch and setBranch are now declared as state
function EditUserModal({ user: u, onClose, onSaved }) {
  ...
  const [branch, setBranch]     = useState(u.branch || '') // FIX #1: was missing
  const [saving, setSaving]     = useState(false)
  const [error, setError]       = useState('')

  async function save() {
    if (role === 'branch_executive' && !branch) { setError('Please select a branch'); return }
    setSaving(true); setError('')
    try {
  ...
  return (
    <div style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:100,display:'flex',alignItems:'center',justifyContent:'center',padding:20 }}>
      <div style={{ background:G.white,borderRadius:20,width:'100%',maxWidth:460,padding:28 }}>
        ...
        <div style={{ display:'grid',gap:16 }}>
          <Field label="Full Name">
            <input type="text" value={fullName} onChange={e=>setFullName(e.target.value)} style={inp} onFocus={e=>e.target.style.borderColor=G.green} onBlur={e=>e.target.style.borderColor=G.border} />
          </Field>
          <Field label="Phone">
            <input type="tel" value={phone} onChange={e=>setPhone(e.target.value)} style={inp} onFocus={e=>e.target.style.borderColor=G.green} onBlur={e=>e.target.style.borderColor=G.border} />
          </Field>
          <Field label="Role">
            <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:8 }}>
              {ROLES.map(r => {
                // FIX #2: safe fallback — ROLE_COLORS[r] might be undefined for custom roles
                const [color, bg] = getRoleColor(r)
                return (
                  <button key={r} type="button" onClick={()=>{ setRole(r); if(r!=='branch_executive') setBranch('') }} style={{ padding:'10px',borderRadius:10,border:`2px solid ${role===r?color:G.border}`,background:role===r?bg:G.white,cursor:'pointer',fontWeight:600,fontSize:13,color:role===r?color:G.muted,textTransform:'capitalize' }}>
                    {r.replace('_',' ')}
                  </button>
                )
              })}
          ...
          {/* FIX #1: branch selector now works — setBranch state exists */}
          {role === 'branch_executive' && (
            <Field label="Assign Branch *">
              <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:8 }}>
                {BRANCHES.map(b => (
                  <button key={b} type="button" onClick={()=>setBranch(b)} style={{ padding:'9px 8px',borderRadius:9,border:`2px solid ${branch===b?'#0891B2':G.border}`,background:branch===b?'#ECFEFF':'#fff',cursor:'pointer',fontSize:12,fontWeight:600,color:branch===b?'#0891B2':G.muted }}>
                    {b}
                  </button>
                ))}
              </div>
      ...
      <div style={{ background:G.white,borderRadius:20,width:'100%',maxWidth:420,padding:28 }}>
        ...
        <div style={{ display:'grid',gap:14 }}>
          ...
          <Field label="Confirm New Password">
            <input type={showPass?'text':'password'} value={confirm} onChange={e=>setConfirm(e.target.value)} placeholder="Re-enter password" style={{ ...inp,borderColor:confirm&&newPass!==confirm?G.red:G.border }} />
          </Field>
        </div>
...
function MyProfile({ currentUser, onUpdated }) {
  ...
  async function saveProfile() {
    ...
    setMsg('Profile updated!'); setSaving(false); onUpdated()
  ...
  async function changeOwnPassword() {
    ...
    try {
      ...
      setPassMsg('Password changed!'); setOldPass(''); setNewPass(''); setConfirm('')
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20 }}>
      <div style={{ background:G.white,borderRadius:16,padding:24,boxShadow:'0 1px 4px rgba(0,0,0,0.06)' }}>
        <h3 style={{ margin:'0 0 20px',fontSize:16,fontWeight:700 }}>My Profile</h3>
        ...
        <div style={{ display:'grid',gap:14,marginBottom:16 }}>
          <Field label="Username"><input type="text" value={currentUser.username} disabled style={{ ...inp,background:'#F9FAF7',color:G.muted,cursor:'not-allowed' }} /></Field>
          <Field label="Full Name"><input type="text" value={fullName} onChange={e=>setFullName(e.target.value)} style={inp} onFocus={e=>e.target.style.borderColor=G.green} onBlur={e=>e.target.style.borderColor=G.border} /></Field>
          <Field label="Phone"><input type="tel" value={phone} onChange={e=>setPhone(e.target.value)} style={inp} onFocus={e=>e.target.style.borderColor=G.green} onBlur={e=>e.target.style.borderColor=G.border} /></Field>
          <Field label="Role"><div style={{ padding:'10px 14px',borderRadius:10,background:'#F9FAF7',fontSize:13 }}><RoleBadge role={currentUser.role} /></div></Field>
        </div>
        <button onClick={saveProfile} disabled={saving} style={{ width:'100%',padding:12,background:saving?'#9CA3AF':G.green,color:G.white,border:'none',borderRadius:12,fontSize:14,fontWeight:700,cursor:'pointer' }}>
          {saving ? 'Saving...' : 'Save Profile'}
        </button>
      </div>
      <div style={{ background:G.white,borderRadius:16,padding:24,boxShadow:'0 1px 4px rgba(0,0,0,0.06)' }}>
        <h3 style={{ margin:'0 0 20px',fontSize:16,fontWeight:700 }}>Change My Password</h3>
        ...
        <div style={{ display:'grid',gap:14,marginBottom:16 }}>
          ...
          <Field label="New Password"><input type={showP?'text':'password'} value={newPass} onChange={e=>setNewPass(e.target.value)} placeholder="Min 6 characters" style={inp} /></Field>
          <Field label="Confirm New Password"><input type={showP?'text':'password'} value={confirm} onChange={e=>setConfirm(e.target.value)} placeholder="Re-enter new password" style={{ ...inp,borderColor:confirm&&newPass!==confirm?G.red:G.border }} /></Field>
        </div>
        <button onClick={changeOwnPassword} disabled={savingPass||!oldPass||!newPass||newPass!==confirm} style={{ width:'100%',padding:12,background:savingPass||!oldPass||!newPass||newPass!==confirm?'#9CA3AF':G.amber,color:G.white,border:'none',borderRadius:12,fontSize:14,fontWeight:700,cursor:'pointer' }}>
...
// ── FIX #3: Password Reset Requests Panel ─────────────────
function ResetRequestsPanel() {
  ...
  async function approve(req) {
    const pw = tempPw[req.id]?.trim()
    if (!pw || pw.length < 6) { alert('Enter a temporary password (min 6 characters)'); return }
    setProc(req.id)
    try {
      const hashed = await hashPassword(pw)
      await supabase.from('profiles').update({ password_hash: hashed }).eq('id', req.user_id)
      await supabase.from('password_reset_requests').update({
        status: 'approved', temp_pw: pw, updated_at: new Date().toISOString()
      }).eq('id', req.id)
      setRequests(prev => prev.map(r => r.id===req.id ? { ...r, status:'approved', temp_pw:pw } : r))
      setTempPw(prev => { const n={...prev}; delete n[req.id]; return n })
  ...
  async function reject(id) {
    ...
    await supabase.from('password_reset_requests').update({ status:'rejected', updated_at:new Date().toISOString() }).eq('id', id)
  ...
  if (loading) return <p style={{ color:G.muted, padding:20 }}>Loading...</p>

  return (
    <div>
      {pending.length > 0 && (
        <div style={{ background:G.amberLight,border:`1px solid ${G.amber}`,borderRadius:12,padding:'12px 16px',marginBottom:20,display:'flex',alignItems:'center',gap:10 }}>
          <span style={{ fontSize:20 }}>🔔</span>
          <p style={{ margin:0,fontSize:14,fontWeight:700,color:G.amber }}>{pending.length} pending password reset request{pending.length>1?'s':''}</p>
        </div>
      )}
      {requests.length === 0 && (
        <div style={{ textAlign:'center',padding:'60px 20px',background:G.white,borderRadius:14 }}>
          <p style={{ fontSize:36,marginBottom:10 }}>🔒</p>
          <p style={{ fontWeight:600,color:G.text,margin:'0 0 6px' }}>No reset requests</p>
          <p style={{ fontSize:13,color:G.muted }}>Password reset requests from users will appear here</p>
        </div>
      )}
      <div style={{ display:'flex',flexDirection:'column',gap:12 }}>
        {requests.map(req => (
          <div key={req.id} style={{ background:G.white,borderRadius:14,padding:18,boxShadow:'0 1px 4px rgba(0,0,0,0.06)',border:`1px solid ${req.status==='pending'?G.amber:req.status==='approved'?G.green:G.border}` }}>
            <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:12 }}>
              <div>
                <p style={{ margin:'0 0 3px',fontWeight:700,fontSize:15 }}>{req.full_name||req.username}</p>
                <p style={{ margin:'0 0 2px',fontSize:12,color:G.muted }}>@{req.username} · 📞 {req.phone||'No phone'}</p>
              ...
              <span style={{ fontSize:11,fontWeight:700,padding:'3px 10px',borderRadius:20,
                background:req.status==='pending'?G.amberLight:req.status==='approved'?G.greenLight:G.redLight,
                color:req.status==='pending'?G.amber:req.status==='approved'?G.green:G.red,
                textTransform:'capitalize' }}>
                {req.status}
              </span>
            </div>
            {req.status === 'approved' && req.temp_pw && (
              <div style={{ background:G.greenLight,borderRadius:10,padding:'10px 14px',marginBottom:12 }}>
                <p style={{ margin:'0 0 4px',fontSize:12,color:G.greenDark,fontWeight:600 }}>✅ Password reset. Tell the user:</p>
                <p style={{ margin:0,fontSize:13,fontFamily:'monospace',fontWeight:700,color:G.greenDark }}>Temporary password: {req.temp_pw}</p>
                <p style={{ margin:'4px 0 0',fontSize:11,color:G.green }}>Ask them to log in and change it immediately.</p>
              </div>
            )}
            {req.status === 'pending' && (
              <div style={{ display:'flex',gap:10,alignItems:'flex-end',flexWrap:'wrap' }}>
                <div style={{ flex:1,minWidth:200 }}>
                  <label style={{ display:'block',fontSize:11,fontWeight:700,color:G.muted,textTransform:'uppercase',letterSpacing:'0.6px',marginBottom:5 }}>Set Temporary Password</label>
                  <input type="text" value={tempPw[req.id]||''} onChange={e=>setTempPw(prev=>({...prev,[req.id]:e.target.value}))}
                ...
                <button onClick={()=>approve(req)} disabled={processing===req.id}
                  style={{ padding:'9px 18px',background:processing===req.id?'#9CA3AF':G.green,color:G.white,border:'none',borderRadius:9,fontSize:13,fontWeight:700,cursor:'pointer',whiteSpace:'nowrap' }}>
                  {processing===req.id?'Processing...':'✓ Approve & Set Password'}
                </button>
                <button onClick={()=>reject(req.id)}
                  style={{ padding:'9px 14px',background:G.redLight,color:G.red,border:'none',borderRadius:9,fontSize:13,fontWeight:600,cursor:'pointer' }}>
                  ✕ Reject
                </button>
              </div>
            )}
...
export default function AdminPage() {
  ...
  // FIX #3: tabs now include Password Resets with a badge count
  const TABS = [
              ...
              <table style={{ width:'100%',borderCollapse:'collapse',fontSize:13 }}>
                <thead>
                  <tr style={{ background:'#F9FAF7' }}>
                    {/* FIX #5: corrected column order — Branch before Phone */}
                ...
                <tbody>
                  {loading && <tr><td colSpan={9} style={{ padding:40,textAlign:'center',color:G.muted }}>Loading users...</td></tr>}
                    ...
                    <tr key={u.id} style={{ borderTop:`1px solid ${G.border}`,background:i%2?'#FAFAFA':G.white }}>
                      ...
                      <td style={{ padding:'12px 14px' }}>
                        <div style={{ display:'flex',alignItems:'center',gap:10 }}>
                          <div style={{ width:32,height:32,borderRadius:'50%',background:G.greenLight,display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:700,color:G.greenDark,flexShrink:0,overflow:'hidden' }}>
                            {u.avatar_url ? <img src={u.avatar_url} alt="" style={{ width:'100%',height:'100%',objectFit:'cover' }} /> : (u.full_name?.[0]||u.username?.[0]?.toUpperCase()||'U')}
                          </div>
                      ...
                      {/* FIX #5: Branch column BEFORE Phone column — matches header order */}
