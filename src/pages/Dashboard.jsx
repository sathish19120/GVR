...
export default function Dashboard() {
  ...
  async function updateOrderStatus(id, status) {
    ...
    if (order) recordAction(id, order.order_number, 'status', { status: order.status }, { status })
                          ...
                          <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                            {o.payment_status==='pending' && o.payment_method!=='cod' && (
                              <button onClick={async()=>{
                                recordAction(o.id, o.order_number, 'payment', { payment_status: o.payment_status }, { payment_status:'paid' })
                                await supabase.from('orders').update({payment_status:'paid'}).eq('id',o.id); load()
                              }}
                            ...
                            {o.payment_method==='cod' && o.payment_status==='pending' && (
                              <button onClick={async()=>{
                                recordAction(o.id, o.order_number, 'payment', { payment_status: o.payment_status, notes: o.notes }, { payment_status:'paid', notes:(o.notes?o.notes+' · ':'')+'Cash collected by admin' })
                            ...
                            {o.status==='dispatched' && o.payment_status!=='paid' && (
                              <button onClick={async()=>{
                                recordAction(o.id, o.order_number, 'payment', { payment_status: o.payment_status, notes: o.notes }, { payment_status:'paid', notes:(o.notes||'')+' · UPI paid on delivery' })
                            ...
                            <button onClick={async()=>{
                              const confirmed = window.confirm(`⚠️ Delete order ${o.order_number}?\n\nYou can Undo this immediately after if it's a mistake — but the Undo history clears if you refresh the page.`)
                              if (!confirmed) return
                              ...
                              recordAction(o.id, o.order_number, 'delete', { ...o, order_items: o.order_items }, null)
