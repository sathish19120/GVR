...
export default function AuthPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (mode === 'signup') {
      ...
      if (ok) { setDone('Account created! You can now sign in.'); switchMode('login') }
    } else {
  ...
  async function checkUsername() {
    ...
    try {
      ...
      if (err || !data) { setFErr('Username not found. Check and try again.'); return }
  ...
  const btn = (bg) => ({
    width:'100%', padding:14, background:bg || G.green,
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', fontFamily:"'Inter',sans-serif" }}>
      <style>{`@media(max-width:640px){.auth-left{display:none!important}.auth-right{width:100%!important;padding:36px 24px!important}}`}</style>
      ...
      <div className="auth-right" style={{ width:460,flexShrink:0,background:G.white,display:'flex',alignItems:'center',justifyContent:'center',padding:'48px 44px',boxShadow:'-4px 0 20px rgba(0,0,0,0.06)' }}>
        <div style={{ width:'100%',maxWidth:340 }}>
          ...
          {mode === 'login' && (
            <>
              <h2 style={{ fontSize:30,fontWeight:800,color:G.text,margin:'0 0 6px' }}>Welcome</h2>
              <p style={{ color:G.muted,fontSize:14,margin:'0 0 28px' }}>Sign in to continue</p>
              ...
              <form onSubmit={handleSubmit}>
                <div style={{ marginBottom:16 }}>
                  ...
                  <input type="text" value={username} onChange={e=>setUsername(e.target.value.trim().toLowerCase())}
                    placeholder="Enter username" required autoComplete="username" style={inp}
                ...
                <div style={{ marginBottom:16 }}>
                  ...
                  <div style={{ position:'relative' }}>
                    <input type={showPw?'text':'password'} value={password} onChange={e=>setPassword(e.target.value)}
                      placeholder="Enter password" required style={{ ...inp,paddingRight:50 }}
                      onFocus={e=>e.target.style.borderColor=G.green} onBlur={e=>e.target.style.borderColor=G.border} />
                    <button type="button" onClick={()=>setShowPw(!showPw)} style={{ position:'absolute',right:12,top:'50%',transform:'translateY(-50%)',background:'none',border:'none',cursor:'pointer',color:G.muted,fontSize:12,fontWeight:600 }}>{showPw?'Hide':'Show'}</button>
                  </div>
                </div>
                <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:22 }}>
                  <button type="button" onClick={()=>switchMode('signup')} style={{ background:'none',border:'none',color:G.green,fontSize:13,fontWeight:600,cursor:'pointer',padding:0,textDecoration:'underline' }}>Create Account</button>
                  <button type="button" onClick={()=>switchMode('forgot')} style={{ background:'none',border:'none',color:G.amber,fontSize:13,fontWeight:600,cursor:'pointer',padding:0,textDecoration:'underline' }}>Forgot Password?</button>
                </div>
          ...
          {mode === 'signup' && (
            <>
              <h2 style={{ fontSize:28,fontWeight:800,color:G.text,margin:'0 0 6px' }}>Create Account</h2>
              <p style={{ color:G.muted,fontSize:14,margin:'0 0 24px' }}>Sign up to start ordering rice</p>

              {error && (
                <div style={{ background:G.redBg,border:'1px solid #FECACA',borderRadius:10,padding:'10px 14px',marginBottom:16,display:'flex',justifyContent:'space-between' }}>
                  <span style={{ color:G.red,fontSize:13 }}>{error}</span>
                  <button type="button" onClick={clearError} style={{ background:'none',border:'none',color:G.red,cursor:'pointer',fontSize:16 }}>✕</button>
                </div>
              )}
              ...
              <form onSubmit={handleSubmit}>
                <div style={{ marginBottom:14 }}>
                  ...
                  <input type="text" value={fullName} onChange={e=>setFullName(e.target.value)} placeholder="Your full name" required style={inp}
                    onFocus={e=>e.target.style.borderColor=G.green} onBlur={e=>e.target.style.borderColor=G.border} />
                </div>
                <div style={{ marginBottom:14 }}>
                ...
                <div style={{ marginBottom:14 }}>
                  <label style={lbl}>Username *</label>
                  <input type="text" value={username} onChange={e=>setUsername(e.target.value.trim().toLowerCase())} placeholder="Choose a username" required style={inp}
                    onFocus={e=>e.target.style.borderColor=G.green} onBlur={e=>e.target.style.borderColor=G.border} />
                </div>
                <div style={{ marginBottom:14 }}>
                  <label style={lbl}>Password *</label>
                  <div style={{ position:'relative' }}>
                    <input type={showPw?'text':'password'} value={password} onChange={e=>setPassword(e.target.value)} placeholder="Min 6 characters" required style={{ ...inp,paddingRight:50 }}
                      onFocus={e=>e.target.style.borderColor=G.green} onBlur={e=>e.target.style.borderColor=G.border} />
                    <button type="button" onClick={()=>setShowPw(!showPw)} style={{ position:'absolute',right:12,top:'50%',transform:'translateY(-50%)',background:'none',border:'none',cursor:'pointer',color:G.muted,fontSize:12,fontWeight:600 }}>{showPw?'Hide':'Show'}</button>
                  </div>
                </div>
          ...
          {mode === 'forgot' && (
            <>
              ...
              {fErr && (
                <div style={{ background:G.redBg,border:'1px solid #FECACA',borderRadius:10,padding:'10px 14px',marginBottom:14,display:'flex',justifyContent:'space-between' }}>
              ...
              {fMsg && (
                <div style={{ background:G.greenLight,border:'1px solid #97C459',borderRadius:10,padding:'10px 14px',marginBottom:14 }}>
              ...
              {fStep === 2 && fProfile && (
                <>
                  ...
                  <div style={{ marginBottom:14 }}>
                    ...
                    <input type="password" value={fNewPw} onChange={e=>setFNewPw(e.target.value)} placeholder="Min 6 characters" autoFocus style={inp}
                      onFocus={e=>e.target.style.borderColor=G.green} onBlur={e=>e.target.style.borderColor=G.border} />
                  </div>
                  <div style={{ marginBottom:20 }}>
                    <label style={lbl}>Confirm New Password</label>
                    <input type="password" value={fConfPw} onChange={e=>setFConfPw(e.target.value)} placeholder="Re-enter password"
                  ...
                  <button type="button" onClick={doReset} disabled={fLoading||!fNewPw||fNewPw!==fConfPw}
                    style={{ ...btn(fLoading||!fNewPw||fNewPw!==fConfPw?'#9CA3AF':G.amber),marginBottom:10 }}>
