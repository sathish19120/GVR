...
// ── Top Nav Modal (standalone component) ─────────────────
      ...
      <div onClick={e=>e.stopPropagation()} style={{ background:'#fff',borderRadius:20,width:'100%',maxWidth:580,maxHeight:'85vh',overflowY:'auto',padding:32 }}>
        ...
        {modal==='what' && <>
          ...
          <p style={{ color:'#6B7280',fontSize:14,lineHeight:1.7,marginBottom:20 }}>
            We are a <strong style={{color:'#3B6D11'}}>direct-to-consumer rice brand</strong> sourcing premium Sona Masoori from Telangana farms, milling fresh, and delivering to your kitchen — no middlemen, maximum freshness.
          </p>
          <div style={{ display:'grid',gap:12 }}>
            {[{icon:'🌱',title:'Farm Sourcing',desc:'Directly from certified paddy farmers in Nalgonda, Khammam, and Warangal districts of Telangana.'},{icon:'⚙️',title:'Fresh Milling',desc:'Milled in small batches. Every pack carries the milling date so you know exactly how fresh your rice is.'},{icon:'📦',title:'Quality Packing',desc:'Available in 1kg and 5kg packs. FSSAI-compliant packaging with best-before dates.'},{icon:'🚪',title:'Doorstep Delivery',desc:'Orders placed through our app are delivered to your home within hours.'},{icon:'💰',title:'Fair Pricing',desc:'By cutting out wholesalers — ₹60/kg for 1kg packs, ₹50/kg for 5kg packs.'}].map(item=>(
        ...
        {modal==='about' && <>
          ...
          <p style={{ color:'#6B7280',fontSize:14,lineHeight:1.8,marginBottom:16 }}>
            Green Village Rice was founded with a simple belief — <em style={{color:'#3B6D11'}}>every family deserves fresh, clean rice at a fair price</em>. We saw that most rice in Hyderabad had been sitting in warehouses for 6–12 months. We decided to change that.
          </p>
          <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:18 }}>
    ...
    <div style={{ minHeight:'100vh', background:G.surface }}>
      ...
      <div style={{ maxWidth:500, margin:'0 auto', padding:'16px 16px 100px' }}>
        ...
        <div style={{ background:G.white, borderRadius:14, padding:18, marginBottom:20, boxShadow:'0 1px 4px rgba(0,0,0,0.06)' }}>
          ...
          {[['cod','💵','Cash on Delivery','Pay when your order arrives'],['upi','📱','UPI Payment','GPay, PhonePe, Paytm'],['bank','🏦','Bank Transfer','NEFT / IMPS / RTGS']].map(([val,icon,label,sub])=>(
        ...
        <button onClick={placeOrder} disabled={placing||!address.trim()} style={{ width:'100%', padding:16, background:placing||!address.trim()?'#9CA3AF':G.green, color:G.white, border:'none', borderRadius:14, fontSize:16, fontWeight:700, cursor:placing||!address.trim()?'not-allowed':'pointer', boxShadow:'0 4px 14px rgba(59,109,17,0.3)' }}>
