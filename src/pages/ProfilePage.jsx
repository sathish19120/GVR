...
export default function ProfilePage({ onClose }) {
  ...
  async function handleImageChange(e) {
    ...
    try {
      // Resize image client-side before upload to keep storage lean
      const resized = await resizeImage(file, 200, 200)

      // Upload to Supabase Storage bucket: avatars/{username}.jpg
      ...
      if (uploadErr) {
        // Bucket may not exist yet — fall back to smaller base64 (resized only)
        // This keeps the row well under 50KB for a 200x200 JPEG
      ...
      setErr('Failed to save image: ' + e.message)
    } finally {
      setUploading(false)
    }
  }

  // Resize image to maxW x maxH, returns a Blob
  ...
  async function changePassword() {
    ...
    try {
      ...
      const hashBuf = async (pw) => {
        const data = encoder.encode(pw + 'gvr_salt_2026')
        const hash = await crypto.subtle.digest('SHA-256', data)
        return Array.from(new Uint8Array(hash)).map(b => b.toString(16).padStart(2,'0')).join('')
      }
      const oldHashed = await hashBuf(oldPass)
      ...
      const newHashed = await hashBuf(newPass)
      const { error } = await supabase
        .from('profiles')
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:300, display:'flex', alignItems:'center', justifyContent:'center', padding:16 }}>
      <div style={{ background:G.white, borderRadius:20, width:'100%', maxWidth:460, maxHeight:'92vh', overflowY:'auto', boxShadow:'0 20px 60px rgba(0,0,0,0.25)' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', marginTop:-40, marginBottom:8 }}>
          ...
          {!uploading && <p style={{ margin:'6px 0 0', fontSize:11, color:G.muted }}>Tap photo to change</p>}
        </div>
        ...
        <div style={{ padding:'0 20px 22px' }}>
          ...
          {err && <div style={{ background:G.redLight, border:`1px solid #FECACA`, borderRadius:8, padding:'8px 12px', marginBottom:12, color:G.red, fontSize:12, display:'flex', justifyContent:'space-between' }}>
            <span>{err}</span><button type="button" onClick={()=>setErr('')} style={{ background:'none',border:'none',cursor:'pointer',color:G.red,fontSize:14 }}>✕</button>
          </div>}
          ...
          {!isReadOnly && (
            <div style={{ background:'#F9FAF7', borderRadius:14, padding:16, marginBottom:14 }}>
              ...
              <button type="button" onClick={changePassword} disabled={savingPass||!oldPass||!newPass||newPass!==confirmPass}
